...
export default defineComponent({
  ...
  setup() {
    ...
    const filteredItems = computed(() => {
      ...
      const filteredGroups = appStore.assistantProfiles.filter((profile) =>
    ...
    const selectItem = (id: string) => {
      appStore.setActiveAssistantOrProfile(id)
      isDropdownOpen.value = false
    }

    const activeItemName = computed(() => {
      if (appStore.activeAssistantProfileId) {
        return (
          appStore.activeAssistantProfile?.name || "Select a group or assistant"
        )
      } else {
